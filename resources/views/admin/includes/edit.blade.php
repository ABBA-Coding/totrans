...
@section('section')
    <div class="pcoded-inner-content">
        ...
        <form action="{{ route('admin.includes.update', ['id' => $data->id]) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
                            ...
                            <div class="tab-pane fade {{ $locale == 1 ? 'show active' : '' }}" id="pills-{{ $key }}" role="tabpanel" aria-labelledby="pills-{{ $key }}-tab">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text"
                                           name="title_{{ $key }}"
                                           class="form-control"
                                           value="{{ old('title_'.$key) ?? $data->{'title_'.$key} }}"
                                           autocomplete="off"
                                           title="">
                                </div>
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <textarea class="form-control"
                                              ...
                                              title="">{!! old('description_'.$key) ?? $data->{'description_'.$key} !!}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
